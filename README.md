...
| [`docs/PHASE-6.md`](docs/PHASE-6.md) | **Phase 6 门店确认/驳回 阶段计划与契约（🟡 进行中，2026-09-25）** —— 起点 = `WAIT_STORE_CONFIRM`；**P6-0 🟢 PASS（阶段已关闭，基线 `0d45b09`）** / **P6-1 🟡 进行中**（契约先行）；子阶段划分；**审核对象钉死 = 当前 SUBMITTED Visit**；**照片访问四边界矩阵**（本店 200 / HQ 200 / 跨店 404 / 匿名 401）+ 反向用例；并发"陈旧页面不得覆盖"规则；**§6.3 只读视图落点 = 内联 H3，不新建动作**；**§12 交付状态表 + §12.1 机器门计数**；复用件盘点（**不改模型**） |
...
| [`docs/PHASE-6-P6-1-CONTRACT.md`](docs/PHASE-6-P6-1-CONTRACT.md) | **P6-1 事务契约（钉死版，2026-09-25 开工）** —— `confirmed_charge_amount` 如何形成 / 驳回原因与 Visit 后续语义 / `FOR UPDATE` 并发 loser 的返回码与响应体 / I12·I13 幂等键与重放语义 / confirm → `WAIT_FEEDBACK` 的评价 Token·短信边界 / reject 之后**究竟停在哪里**等待重新处理；含**契约自检清单**与**待用户裁决的开放项** |
...
| Phase 6 门店确认 / 驳回 | 🟡 **进行中（2026-09-25 启动）** —— 计划/契约已定稿（`docs/PHASE-6.md`）。**P6-0** = Store Review Read Model & **Photo Access Gate**（只做读 + 私有照片受控访问，**不写**确认/驳回）→ 🟢 **PASS（阶段已关闭 2026-09-25，功能交付基线 `0d45b09`）**：§5 四边界矩阵 **24/24 正向 + 9/9 反向**（本店 200 / HQ 200 / 跨店 404 / 匿名 401）、N1 原生口封禁（顺带修掉 **DEV-83**：门店角色原生读 `serviceVisitPhotos` 曾返回 200 且下发 `storage_key`）、真实浏览器闸门 §3.7 第③层「照片真的解码」**绿**、smoke **118/118**、**U1 人眼项已执行一次并通过**（3 张工单，真实 Chromium；取证边界见 `docs/PHASE-6-P6-0-UAT-SHEET.md` §六）。证据 `docs/PHASE-6-P6-0-EVIDENCE.md`。**P6-1** = confirm/reject 事务（落 M9/M10）→ 🟡 **进行中，契约先行**（`docs/PHASE-6-P6-1-CONTRACT.md`）。**不重新设计 Ticket/Visit 模型** |
