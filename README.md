...
| [`docs/PHASE-6.md`](docs/PHASE-6.md) | **Phase 6 门店确认/驳回 阶段计划与契约（⬜ 计划态，2026-09-25 定稿）** —— 起点 = `WAIT_STORE_CONFIRM`；子阶段 **P6-0**（Store Review Read Model & **Photo Access Gate**，只做读）/ **P6-1**（confirm/reject 事务）；**审核对象钉死 = 当前 SUBMITTED Visit**；**照片访问四边界矩阵**（本店 200 / HQ 200 / 跨店 404 / 匿名 401）+ 反向用例；并发"陈旧页面不得覆盖"规则；复用件盘点（**不改模型**） |
...
| Phase 6 门店确认 / 驳回 | 🟡 **进行中（2026-09-25 启动）** —— 计划/契约已定稿（`docs/PHASE-6.md`）。**P6-0** = Store Review Read Model & **Photo Access Gate**（只做读 + 私有照片受控访问，**不写**确认/驳回）；**首个硬问题 = 照片读取权限**（四边界：本店 200 / HQ 200 / 跨店 404 / 匿名 401）。P6-1 = confirm/reject 事务（落 M9/M10）。**不重新设计 Ticket/Visit 模型** |
