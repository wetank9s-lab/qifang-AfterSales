...
| [`docs/PHASE-5.md`](docs/PHASE-5.md) | **Phase 5 阶段计划（🟡 HOLD；P5-0/P5-1 已 PASS，P5-2 PENDING）** —— 师傅 H5：阶段状态表、目标与范围、复用件盘点、交付物清单、**五个关键设计决策**、**Token 失效矩阵 6 条硬验收**、照片 1–6 张口径、M8 提交与状态机、反向验证清单、前置核查闸门、风险与限制、**§13 P5-2 收口验收** |
...
| [`docs/PHASE-5-P5-2-UAT.md`](docs/PHASE-5-P5-2-UAT.md) | **P5-2 手机真人走查方法（⬜ PENDING）** —— 从短信形状 `/t/{token}` 出发的**普通技师视角**验收、**禁止预提示**、锁定的 PASS 门槛、**五个阻断项 vs 视觉 backlog**、**三个自然问题**、数据复核与清理 |
...
| Phase 5 师傅 H5 | 🟡 **HOLD**（2026-09-23 启动；分包推进，见 `docs/PHASE-5.md`）—— **P5-0 Routing & Environment Gate ✅ PASS**（`7b7e232`）· **P5-1 Technician API/H5/Security 🟢 PASS**（正式基线 **`297e728`**，2026-09-25 裁定）· **P5-2 Mobile Human UAT ⬜ PENDING**（`docs/PHASE-5-P5-2-UAT.md`）。硬验收 = **Token 失效矩阵 6 条**（`401 TOKEN_INVALID`，HTTP 层取证） |
