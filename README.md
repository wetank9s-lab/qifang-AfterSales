...
| [`docs/PHASE-6.md`](docs/PHASE-6.md) | **Phase 6 门店确认/驳回 阶段计划与契约（P6-0 机器门 🟢 全绿，2026-09-25）** —— 起点 = `WAIT_STORE_CONFIRM`；子阶段 **P6-0**（Store Review Read Model & **Photo Access Gate**，只做读）/ **P6-1**（confirm/reject 事务）；**审核对象钉死 = 当前 SUBMITTED Visit**；**照片访问四边界矩阵**（本店 200 / HQ 200 / 跨店 404 / 匿名 401）+ 反向用例；并发"陈旧页面不得覆盖"规则；**§6.3 只读视图落点 = 内联 H3，不新建动作**；**§12 交付状态表 + §12.1 机器门计数**；复用件盘点（**不改模型**） |
| [`docs/PHASE-6-P6-0-EVIDENCE.md`](docs/PHASE-6-P6-0-EVIDENCE.md) | **P6-0 交付证据（机器门 🟢 全绿 · 关闭待人眼项裁定）** —— §5 冻结矩阵逐条实测值：① 四边界 B1~B5（**24/24**）② N1 原生口封禁（含 `storage_key` 泄漏的 **DEV-83**）③ R1/R2/S1/O1 ④ U1 机器层 + 人眼层 ⑤ **真实浏览器闸门 §3.7 第③层**（照片真的解码）⑥ **本轮三处假红的根因与反向验证** ⑦ 停止线（P6-0 不含写操作）⑧ 机器门计数 |
| [`docs/PHASE-6-P6-0-UAT-SHEET.md`](docs/PHASE-6-P6-0-UAT-SHEET.md) | **P6-0 现场记录表（打印/对照用一页版）** —— U1 人眼项清单：9 条"应当出现" + 4 条"**不应出现**的写入口" + 5 条阻断项 + 结论签字 |
...
| Phase 6 门店确认 / 驳回 | 🟡 **进行中（2026-09-25 启动）** —— 计划/契约已定稿（`docs/PHASE-6.md`）。**P6-0** = Store Review Read Model & **Photo Access Gate**（只做读 + 私有照片受控访问，**不写**确认/驳回）→ **机器门 🟢 全绿**：§5 四边界矩阵 **24/24 正向 + 9/9 反向**（本店 200 / HQ 200 / 跨店 404 / 匿名 401）、N1 原生口封禁（顺带修掉 **DEV-83**：门店角色原生读 `serviceVisitPhotos` 曾返回 200 且下发 `storage_key`）、真实浏览器闸门 §3.7 第③层「照片真的解码」**绿**、smoke **118/118**；证据 `docs/PHASE-6-P6-0-EVIDENCE.md`。**剩 1 项 = U1 人眼走查**（`docs/PHASE-6-P6-0-UAT-SHEET.md`），**关闭待用户裁定**。P6-1 = confirm/reject 事务（落 M9/M10）。**不重新设计 Ticket/Visit 模型** |
