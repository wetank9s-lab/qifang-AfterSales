...
### Phase 8 🟢 PASS（2026-09-26 · `94fa3db`）· 后台任务可靠性 + SMS 失败恢复闭环
...
> 关闭 = 冻结矩阵 **11/11 全 PASS**：10 条机器门 + **第 11 条 U1 人眼项已实际执行一次并通过**。
> **关闭未修改任何已通过的读取实现** —— 本条之后的改动是**文档-only**（外加 P6-1 契约文档）。

**U1 人眼项执行结果（2026-09-25）**
- 门店 A 账号在**真实 Chromium** 中实际登录 → 实际点开 **3 张** `WAIT_STORE_CONFIRM` + `SUBMITTED` 工单的行内「详情」。
- 四项判据全中：① 「技师回执」区块 **3/3 出现**；② 服务结果（中文「已解决」）/ 处理说明（**`NULL` · 长文本 · 短文本三种形态**）/
  是否收费（是 → `¥128.50`、否 → `不收费`）/ **真图非裂图**（DOM `blob:` 图 `naturalWidth>0`，**1/1/2 张全解码**）齐备；
  ③ "技师已提交、等待门店审核"语义可读；④ **无任何**确认 · 驳回 · 金额输入控件。**阻断项命中 0 条**。
- **取证边界（已如实记入 `docs/PHASE-6-P6-0-UAT-SHEET.md` §六）**：
  9 张分段截图**像素未被回读** ⇒ 「非裂图」的依据是 `naturalWidth>0`（**裂图该值必然为 0**）
  + `uat-preflight` §3.7 的独立证据，**不是**"人眼逐张看过照片内容"；
  §2.3 的"越权猜 URL / 加载计时"两项**未执行**，**不计入**判定、**不得**写成已验。
  走查由**本项目自己**执行（**非**第三方售后人员到场）；用户明确**不重跑第二轮真人 UAT**。

**Docs**
- `docs/PHASE-6.md`：头部状态 → 🟡 进行中；**§12 状态表** P6-0 → 🟢 PASS（基线 `0d45b09`）/ U1 → ✅ PASS / P6-1 → 🟡 进行中。
- `docs/PHASE-6-P6-0-EVIDENCE.md`：状态 → 🟢 PASS · 阶段已关闭；§④ 人眼层 → **已执行并通过（含取证边界）**；§⑨ 结论 **11/11**。
- `docs/PHASE-6-P6-0-UAT-SHEET.md`：状态 → ✅ U1 PASS；§二 逐条实测回填；§四 签字；**新增 §六 走查执行记录**。
- `README.md` / `docs/DEV-PLAN.md`：阶段进度表与文档索引同步至 P6-0 PASS。
- **新增 `docs/PHASE-6-P6-1-CONTRACT.md`** —— P6-1 事务契约**钉死版**（**契约先行，不先做按钮**）。
...
> ~~**阶段关闭待用户裁定**~~ —— ✅ **已于 2026-09-25 关闭为 🟢 PASS**（见本文件上方
> 「Phase 6 · P6-0 🟢 PASS —— 阶段已关闭」条目；基线 `0d45b09`）。
> 下句为当时的原始状态陈述，保留不改：
> 冻结矩阵 11 条里 10 条机器可证且全绿，第 11 条 **U1 是明文"人眼验证"**。
