...
> **状态：🟢 PASS —— Phase 4 已关闭，允许进入 Phase 5**（2026-09-23 复核方裁定）
>
> | 部分 | 裁定 |
> |---|---|
...
> | **H 后台页面 + H3/H6 客户端代码** | ✅ **已交付，并随 I 走查一并确认** —— 四张页面已落库；详情抽屉与五个业务按钮（详情 / 受理 / 派工 / 改派 / 改约）**真实渲染**；两处接口契约缺陷（`service_mode` 枚举 / `X-Request-Id`）已于 2026-09-21 收口（§13.4.2） |
> | **I 真人 UI 走查** | ✅ **PASS** —— 共四轮：首轮 **BLOCKED**（自定义动作未挂载，DEV-68/69）→ 第二轮 **P0×1 + P1×2**（DEV-70~73）→ 第三轮 改派 ✅ / 改约 ✅ / **详情 ❌ 404**（DEV-74 产物交付链）→ **第四轮「详情」复测 PASS** |
>
> 2026-09-21 服务层与 action 层完成 · 2026-09-21 `smoke-test.mjs` §4d 共 16 条断言真机全绿 · 2026-09-21 收口 DEV-47 三处"注释正确、代码不对" · 2026-09-21 复核方裁定：服务层 PASS / DEV-45 接受 / 整体 HOLD · **2026-09-23 复核方裁定：Phase 4-I PASS ⇒ Phase 4 关闭，允许进入 Phase 5**
>
> ⚠️ **收尾时如实记录的一点（不得用结论掩盖缺失证据）**：`docs/PHASE-4-I-UAT-SHEET.md` 第四节
> 「三个必答问题（逐字记录）」及前三轮的现场记录栏，**未在本次会话中回收**；
> 本阶段的 PASS 由复核方给出判定。若需纸质留档，请补录该表 —— **在补录之前，不得把该表描述为"已逐字留档"**。
>
...
>
> ✅ **本阶段已按 Phase 4 强制条款关闭**（2026-09-23）：后台业务页面代码（我的门店工单 / 全量工单 /
> 工单详情含时间线 + Visit 区块）已交付，**真实售后人员 UI 走查已完成并判定 PASS**，
> 因此**允许进入 Phase 5**。四轮走查的整改全过程见 §12 / §13.5 与 `docs/DEVIATIONS.md` 的 DEV-68~74。
...
> ⚠️ **2026-09-23 更新（第三轮口径）**：第 1/2/3/7/8 步**已由前两轮真人走通**，本轮**不再重验**；
> 第二轮唯一判出问题的是**第 4 步（改派）**、以及第 5/6 步的交互形态。
> 所以当前轮次是**整改后的定向复测**：只复测"改派+原因 / 预计上门日期 / 详情能否回答三个问题"，
> 并顺带反证门店隔离。**本轮以 `docs/PHASE-4-I-UAT.md` 为准**（下表是原始脚本，保留作历史口径）。
...
| 5 | 门店售后账号 | 对 Visit #2 **改约**（改**预计上门日期**） | **Visit 数量不增加**（仍为 2 条） |
| 6 | 门店售后账号 | 打开**工单详情** | 能**自己说清**：当前谁在处理 / 预计哪天上门 / 之前发生过什么 |
...
> **当前结论（2026-09-21 裁定 + 2026-09-23 收尾裁定）**：
> · **服务层 A~G + 总闸 J = ✅ PASS**（总闸全绿含 §4d 16 条；提交 `453570e`）；
> · **DEV-45 = ✅ 已接受偏差** —— 保留 `200 + {valid:false}`，**无代码改动**；`401` 归还给 Phase 5 认证接口；
> · **H 后台页面 = ✅ 已交付** —— 四张页面已落库（§13.1），工单详情**降级交付**（§13.2，只读抽屉）、
>   业务按钮（H6）真实渲染；**H6 的两处接口契约缺陷（`service_mode` 枚举 / `X-Request-Id` + request-id 幂等）已收口**（§13.4.2）；
> · **I 真人 UI 走查 = ✅ PASS（四轮走查）** —— 首轮 BLOCKED（DEV-68/69 自定义动作未挂载）→ 第二轮 P0×1 + P1×2（DEV-70~73）
>   → 第三轮 改派 ✅ / 改约 ✅ / **详情 ❌ 404**（DEV-74 产物交付链）→ **第四轮「详情」复测 PASS**；
> · **⇒ 阶段整体 🟢 PASS，允许进入 Phase 5**（2026-09-23）
...
| 事项 | 落地物 | 说明 |
|---|---|---|
| 走查记录表 | **`docs/PHASE-4-I-UAT.md`**（+ 一页版 `docs/PHASE-4-I-UAT-SHEET.md`） | 环境事实表 + **本轮定向复测表** / 前几轮的 8 步留档 + 观察项 + 问题整改表 + 结论签署 + 走查后清理 |
...
> 只落本机 `.env` 与 stdout，**绝不入库**。验收后按 **`--disable` 回收**（撤角色/撤映射/重置口令，
> **保留 `users` 行**，否则工单 `handler_user_id` 与事件 `operator_user_id` 的引用会变成空引用 ——
> 证据还在但"说不清是谁做的"；详见 `docs/PHASE-4-I-UAT.md` §9）。
>
...
| 项 | 交付内容 | 状态 |
|---|---|---|
| 只读详情抽屉（H3） | `src/client/ticket-drawer.tsx` —— 点一行打开侧滑抽屉。<br>**当前形态（2026-09-23 整改后，四层）**：<br>**① 摘要**（工单号 + 中文状态 + 品类/工单类型/创建时间；**不含**任何内部字段）<br>**② 客户与问题**（客户姓名 + 脱敏手机号 + 问题描述）<br>**③ 当前服务**（**仅当前活跃 Visit**：服务方式、厂家/第三方名称、当前师傅与电话（按隐私规则）、预计上门日期、状态中文）<br>**④ 处理记录时间线**（历史 Visit 与 `TicketEvent` **合并去重**成一条时间线：谁 / 什么时候 / 做了什么 / 关键变化）<br>⚠️ 旧形态是"基本信息 / 时效 / 派工历史 / 事件时间线"四块 —— 其中"派工历史表"与"事件时间线"是**同一件事说了两遍**，已合并（详见 §13.4.B） | ✅ 代码已交付<br>🟡 **UI 待第三轮真人复测** |
| 4 个业务动作（H6） | `src/client/ticket-actions.tsx` —— 受理 / 派工 / 改派 / 改约，各注册一个客户端 `ActionModel`，只 POST 既有 `/api/svc:*`。**每个动作有各自的参数配置**（字段 / 必填复核 / 载荷构造），不共用（DEV-70） | ✅ 代码已交付<br>🟡 **UI 待第三轮真人复测** |
...
**并且只显示「与当前动作相关的那一条」**（2026-09-23 整改）—— 不再平铺一整墙指标：

| 工单状态 | 显示的那一条 |
|---|---|
| `NEW` | 等待受理多久 |
| `PROCESSING` | 预计哪天上门 |
| `WAIT_STORE_CONFIRM` | 等待门店确认多久 |
| `WAIT_FEEDBACK` | 等待客户评价多久 |
| `CLOSED` | 完成时间 / 总耗时 |

取不到值时退化成一句**不撒谎**的话（例如"已受理，尚未派工"），
**绝不**显示"已等待 0 分钟"去误导售后同事（那会让人以为工单刚建）。
...
> ⚠️ **为什么这里标 🟡 而不是 ✅**：本批代码**没有经过真人验证**。
> 已自动验证的有：
> ① 客户端纯逻辑（状态矩阵 / 时效文案 / **写请求契约与派工参数契约** /
>    **详情文案与隐藏字段纪律**）由 `verify-client-logic.mjs` **50 项**覆盖；
> ② 产物 **HTTP 200** 且 AMD 依赖全部在"运行时可解析"白名单内（防整页 App error）；
>    §4f 另从**已部署产物**里回读派工选项与 `X-Request-Id` 装配，防"源码对了产物没重新构建"；
> ③ `svc:visits` 的服务端契约（`verify-reassign-contract` **A7**，自带 Visit 夹具）；
> ④ 详情抽屉**在真实浏览器里能打开并取到数据**（前哨 **§3.7**）。
> **"抽屉读起来顺不顺、按钮点不点得动"必须由真人走查确认** —— 这正是 I 存在的理由。
...
| 项 | 结论 |
|---|---|
| **管理员口令**（公开仓库不得有弱口令 fallback） | ✅ **已关闭**。三处 `envValue('SMOKE_ADMIN_PASSWORD','admin123')` 全部删除，未配置一律 `exit 2`（环境未就绪）；`.env.example` 只保留**空注释坑位**（`# SMOKE_ADMIN_PASSWORD=`）；实际管理员口令**已轮换**，`admin123` 现返回 **401**。全仓 `grep -rn "admin123"` 零残留 |
| **编辑/删除按钮的越权风险** | ✅ **核实为纯 UX 问题，不是安全漏洞**。实测两业务角色对 `serviceTickets` **只有 `get`/`list`/`view`**，**无 `update`/`destroy`** —— 点下去必被服务端拦。UX 误导线仍按 DEV-53 坑 2 处理：**若真人被误导，须在关闭 Phase 4 前隐藏或改造**（本轮盲测项，见 `docs/PHASE-4-I-UAT.md` §5①） |

**验收方法论的固化**（本轮新增，后续后台 UI 功能统一沿用）：

> **代码层**（模型/逻辑存在）→ **结构层**（真实 `flowModels` 挂载正确）
> → **呈现层**（浏览器真实渲染）→ **业务层**（真人完成完整任务）
>
> 即：`ActionModel 注册 ≠ ActionModel 实例存在 ≠ 用户页面真实可操作`。
> 前三层必须由自动化断言覆盖，**第四层只能由真人给出**，二者不可互相替代。

> ⚠️ 仍标 🟡 的原因：**H3/H6 的动作链必须在整改后的完整版本上重新走查**。
> 首轮已通过的数据隔离结果保留为历史证据。**Phase 4 在此之前继续 HOLD。**
> 走查口径与锁定的 PASS 门槛见 **`docs/PHASE-4-I-UAT.md`**（当前为**第三轮 · 整改后定向复测**）。
>
> 📌 **最终结论（2026-09-23）**：整改后的动作链**已重新走查并通过**（第二轮 P0/P1 ⇒ 第三轮「详情 404」
> ⇒ DEV-74 修复 ⇒ 第四轮复测 PASS）⇒ **Phase 4 已关闭为 🟢 PASS**。上段为该时点的原始记录，保留不改。

#### 13.4.B 第二轮真人走查的判定与整改（2026-09-23）

第二轮真人走查（在 DEV-68/69 整改之后、动作链首次真正可点）给出的判定是
**P0×1 + P1×2**。复核方明确**限定范围**：只修这三项，**不做范围扩展、不重新开发功能**。

| 级别 | 问题 | 根因 | 整改 | 留档 |
|---|---|---|---|---|
| **P0** | **改派填了原因仍 `MISSING_REASON`**（界面表现"填了没用"） | 客户端载荷**按白名单挑字段**构造，而改派复用了「派工」的构造器 —— 它只遍历 5 个派工字段，**不含 `reason`** → `reason` 在**出口**被静默剔除。服务端行为一直是对的 | 改派有**自己的**字段集（`REASSIGN_FORM_FIELDS`）与构造器；客户端由"一个布尔猜动作"改为**每个动作各自的参数配置**（fields / validate / payloadOf） | **DEV-70** |
| **P1** | **「预计上门时间」是分钟级伪精度** —— 项目不采集签到/到达/GPS/精细排程，那个时分无人履约也无人校验，却会被读成"师傅 9 点半到" | 用精度伪装确定性 | 派工与改约 UI 一律改**「预计上门日期」**（日期选择器）；DB **不迁移**（仍 `datetime`），落库统一归一到**当天 12:00（+08:00）**；**UI 不显示**该时刻、**事件/短信不描述**该时刻、**SLA 不据此判定**（逾期口径留 Phase 9） | **DEV-71** |
| **P1** | **详情抽屉按"数据表"平铺，回答不了售后同事真正要问的三件事** | 信息层级按数据结构组织，而不是按**要回答的问题**组织 | 抽屉重构为四层：**摘要**（工单号 + 中文状态 + 品类/类型/创建时间，不含内部字段）/ **客户与问题**（姓名 + 脱敏手机号 + 问题描述）/ **当前服务**（**仅当前活跃 Visit**）/ **处理记录时间线**（历史 Visit 与事件**合并去重**）；内部 id、token 哈希、请求号、内部枚举一律**默认隐藏** | **DEV-72** 同批 |

**整改过程中额外发现并修掉的 P0 级缺陷（不在上述三项之内，但同属"不改就不能验收"）**：

| 缺陷 | 现象 | 根因 | 留档 |
|---|---|---|---|
| **详情抽屉永远"加载失败"** | 抽屉里的请求写成了 `/api/svc:timeline`，而注入的 `request` 走 `app.apiClient.request()` —— **它会自己补 `/api`** → 真实请求 `/api/api/svc:timeline` → **404** `api resource does not exist`（`docker logs` 实测 6 次）。**所有** HTTP / 结构 / 契约断言全绿，因为**没有任何断言看浏览器真正发出的 URL** | 调用方拼错，而断言只测服务端 | 路径改为**不带前缀**的相对名；新增离线断言（**先剥注释再扫**，否则抽屉里解释本坑的注释会导致假红）+ 前哨 **§3.7 真实渲染闸门**。**DEV-72** |
| **`smoke-test` 唯一覆盖 `svc:visits` 的断言被"跳过却算作通过"** | 汇总写「smoke-test 全部通过：117 项」，其中 1 项实际**未验证**（洁净基线 Visit=0 触发跳过），而 `check()` 把"不抛异常的返回"一律计为 `passed++` | ① 跳过与通过**共用计数器**；② 该断言的前置条件要靠**别处残留数据**满足（偶然绿） | 引入 `SkipCheck` 哨兵 + 独立计数：跳过**单独打印、不计入通过**（写入「通过 116 项 · 跳过 1 项」）；同主题断言搬到 `verify-reassign-contract` 的 **A7**（该脚本**自带 Visit 夹具**）。**DEV-73** |

**本轮新增/变更的自动化**：

| 脚本 | 变化 |
|---|---|
| `scripts/verify-reassign-contract.mjs` | **新增**（11 项 + `--reverse` 6 项）。A1~A2b 静态契约（字段集互不污染）、A3 绕过客户端仍 422（**证明没放宽服务端**）、A4 用**与 UI 完全一致的载荷**真打 `svc:reassign`、A5 事件保留原因、A6a~A6d 日期规范化四证、**A7** `svc:visits` 读取契约（ticket 作用域 + 凭据列不出现 + 失效原因保留 + 历史 Visit 可读） |
| `scripts/verify-client-logic.mjs` | 36 → **50 项**：状态→中文、隐藏字段纪律、时效**只显一条**、抽屉请求路径**不带 `/api` 前缀**、改派字段集与 UI 载荷镜像一致 |
| `scripts/uat-preflight.mjs` | 18 → **21 项**：新增 **§3.7 H3 抽屉真实渲染闸门**（点开详情、读四块内容） |
| `scripts/uat-reset-baseline.mjs` | 加入 `verify-reassign-contract`；复位拆为"删噪声 → 夹具打回 `NEW`"两步；**全绿时不再打印误导性的"含 N 处限流迹象"**（该提示原先扫全量输出，而 smoke 自己就有断言 429 的绿灯用例，导致每轮必报） |
| `smoke-test.mjs` | 跳过走独立 `SkipCheck` 分支，**不并入"通过"** |
...
> ⚠️ **2026-09-23 更新**：首轮/第二轮的走查口径见 `docs/PHASE-4-I-UAT.md`
> （**当前为第三轮 · 整改后定向复测**，只复测"改派+原因 / 预计上门日期 / 详情能否回答三个问题"）。
> 下面 4 条中，第 1 条已由第二轮证伪（**未造成障碍** → 记 UX backlog），其余保留。

1. **误点写按钮**：列表上的"编辑 / 删除"是编译器自动合并的（§13.2），
   门店角色点下去必被服务端拦 —— **已核实为纯 UX 问题，不是安全漏洞**
   （两业务角色对 `serviceTickets` 只有 `get`/`list`/`view`，无 `update`/`destroy`）。
   第二轮真人走查**未被这两个按钮带偏** → 记 UX backlog，**不阻塞 Phase 4**；
   若后续再被误导，升级为阻塞项并在关闭 Phase 4 前隐藏或改造。
   ⚠️ **服务端 ACL 不得为了"让按钮看起来有用"而放宽。**
...
4. **字段名是否要培训**：`service_mode`、`provider_name`、`superseded_at` 这些列名
   ...
   （2026-09-23：`service_mode` / `provider_name` 在详情抽屉里已改为**中文口径**
   + "已作废"这类业务说法；内部枚举不再直接暴露。）
...
## 14. 下一阶段（**前置已满足：Phase 4-H / I 均已关闭**）

> ✅ **Phase 5 已获准开始**（2026-09-23 复核方裁定：Phase 4-I PASS ⇒ Phase 4 PASS）。
> H（后台页面）与 I（真人 UI 走查）均已关闭，本阶段的所有阻塞项已清零。
> 走查全过程见 §12.2 / §13.5；Phase 5 的阶段计划见 `docs/PHASE-5.md`。
