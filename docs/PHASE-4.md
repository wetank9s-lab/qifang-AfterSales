...
> **状态：🟡 部分交付（服务层 A~G 与总闸 J 已完成并真机验收；后台页面 H 与 UI 走查 I 未交付）**
> 2026-09-21 服务层与 action 层完成 · 2026-09-21 `smoke-test.mjs` §4d 共 16 条断言真机全绿 · 2026-09-21 收口 DEV-47 三处"注释正确、代码不对"
>
...
>
> ⚠️ **本阶段按强制条款 1/2 尚未关闭**：后台业务页面（我的门店工单 / 全量工单 / 工单详情含时间线 + Visit 区块）
...
## 9. 「改派后旧 Token 失效」的表达形态（DEV-45，**待复核方裁定**）

| 项 | 内容 |
|---|---|
| 条款原文 | `docs/DEV-PLAN.md` Phase 4 步骤 J 写明本阶段断言「含**改派后旧 Token 401**」 |
| 实际交付 | 探针 `svc:tokenCheck` 返回 **HTTP 200** + `{ valid: false, code: 'TOKEN_INVALID' }` |
| 为什么不是 401 | `tokenCheck` 是**总部排障设施**，回答的是"这个 Token 有效吗"这个**问题**，而非"我要用这个 Token 通过认证"。若返回 401，总部运维将无法区分"被问的 Token 无效"与"**我自己的登录态过期了**" —— 两者都长成 401，处置方式完全不同。真正的 401 语义属于 **Phase 5** 的 `GET /api/technician/visits/:token` |
| 语义等价性 | ① 被问的 Token 由 `TokenService.verify()` 判定，**与未来师傅端接口是同一个函数**；② 失败一律 `TOKEN_INVALID`，**内部 reason 不外露**；③ 响应体逐字搜索**找不到** `reassigned` 等字样（冒烟有专门断言钉这一点） |
| 需裁定 | 是否接受"**HTTP 层已证明失效、但状态码是 200**"作为本条款的达成证据？若不接受，最小改法 = `valid:false` 时返回 401（代价：失去上面那条排障区分能力）。**本项在获得明确裁定前按当前形态保留** |
...
| DEV-45 | 「改派后旧 Token 失效」的形态是 `200 + {valid:false}` 而非 401（**待复核方确认**） |
...
| 项 | 说明 |
|---|---|
| **后台业务页面（H）** | 我的门店工单（状态 Tab）/ 全量工单 / 工单详情（含事件时间线区块）/ Visit 审核区块，**未交付** |
| **真实售后人员 UI 走查（I）** | 派工 / 改派 / 改约的浏览器操作走查**未进行**，走查人与走查时间**未记录** |
| `storeUsers` 门店用户映射**种子** | 仍等业务方给账号清单；当前仅验收期临时建号 |

---

## 12. 强制条款（`docs/DEV-PLAN.md` §「Phase 4 的强制交付条款」）

1. **后台业务页面必须在本 Phase 完成前交付**：我的门店工单（状态 Tab）/ 全量工单 / 工单详情（含事件时间线区块）/ Visit 审核区块。
   Phase 2 的该缺口**只允许重排期到本 Phase，不允许继续顺延**。 → **当前未完成**
...
> **当前结论**：Phase 4 的**服务层与总闸验收已完成**（A~G + J），但 H / I 未交付。
> 按强制条款 1/2/3，**Phase 4 判定为「未关闭」，不得进入 Phase 5**。
> 本报告的服务层部分可作为 H 步的规格依据（页面要展示的状态、Visit 字段、事件时间线均已定型）。
...
Phase 4 关闭后进入 **Phase 5 — 师傅 H5**：`/technician/visit/:token` 页面；
`GET /api/technician/visits/:token`；`POST .../files`（私有存储 + magic bytes + 去 EXIF + 受控读取）；
`POST .../submit`（收费校验）；提交后置 `WAIT_STORE_CONFIRM` 且**不发评价短信**。
