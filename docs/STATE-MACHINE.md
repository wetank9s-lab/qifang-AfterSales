...
| M4 | `reassign` | PROCESSING | PROCESSING | 存在未提交的 Visit | **旧 Token 立即失效**；同一 Visit 换师傅/换手机号并**新建新 Token**；发更新短信给客户 + 新师傅；写 `reassigned` |
| M5 | `reschedule` | PROCESSING | PROCESSING | 同上 | **旧 Token 失效 → 新 Token**；改 `expected_visit_at`；发更新短信；写 `rescheduled` |
...
| M8 | `technicianSubmit` | PROCESSING | WAIT_STORE_CONFIRM | Token 有效 + 未使用 + 未过期 + 绑定的 Visit 属于该 ticket 且 `store_confirm_status=pending`；`service_result`/`service_note` 必填；`is_charged=true → amount>0`，`false → amount=0` | 写 Visit 回执；`token_used_at` 置位（**Token 立即失效**）；`store_confirm_status=pending`；**绝不发客户评价短信**；写 `technician_submitted` |
