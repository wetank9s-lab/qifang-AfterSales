# Phase 5 — 师傅 H5（阶段交付报告）

> **状态**：🟢 **PASS —— Phase 5 已关闭，允许进入下一阶段**（用户 2026-09-25 裁定）
>
> | 子阶段 | 裁定 | 基线 |
> |---|---|---|
> | **P5-0** Routing & Environment Gate | ✅ **PASS** | commit `7b7e232` |
> | **P5-1** Technician API / H5 / Security | 🟢 **PASS** | **正式交付基线 `297e728`**（用户 2026-09-25 裁定） |
> | **P5-2** Mobile Human UAT | 🟢 **PASS** | **DEV-82 收口修复 `14c5b1a`** |
> | **Phase 5** | 🟢 **PASS** | 允许进入下一阶段（Phase 6：门店确认 / 驳回） |
>
> **裁定原文**：**「Phase 5：🟢 PASS / P5-2：🟢 PASS / Phase 5：🟢 PASS / 允许进入下一阶段。」**
> —— DEV-82 已把真人 UAT 唯一留下的条件项完整收口，证据链足够（服务端权威规则 +
> 前端消费服务端规则 + 19 项 submit 矩阵 + 反向/变异验证 + 真实 Chromium 空说明提交链路），
> **不再安排第二轮完整真人 UAT**。
>
...
| **P5-0** Routing & Environment Gate | ✅ **PASS** | commit `7b7e232` |
| **P5-1** Technician API / H5 / Security | 🟢 **PASS** | **正式交付基线 `297e728`**（用户 2026-09-25 裁定） |
| **P5-2** Mobile Human UAT | 🟢 **PASS** | 真人走查通过 + **DEV-82** 条件项收口（`14c5b1a`）→ `docs/PHASE-5-P5-2-UAT.md` |
| **Phase 5** | 🟢 **PASS** | **阶段关闭**；下一步从 `WAIT_STORE_CONFIRM` 接棒（Phase 6） |

> ⚠️ **口径**：P5-1 关闭**不等于** Phase 5 关闭。机器已证明系统正确；
> P5-2 要验的是**人能不能正确理解系统**——即"机器全绿"之外的最后一道人类验收。
>
> **P5-2 的通过路径（如实记录，不美化）**：手机真人在**首次走查**中即通过入口 / 上传 / 收费 /
> 提交 / 一次性 Token 失效五个动作，**无操作阻断**；同时提出一条**规则反馈** ——
> `resolved` 场景下"处理说明"不应强制填写。该反馈立为 **DEV-82**（条件必填，见 §4.5），
> 经**定向机器验证** + **最小真实浏览器复验**收口后，由用户裁定 **P5-2 🟢 PASS**。
> ⇒ 全程**只做过一轮真人走查**，DEV-82 之后**没有**第二轮完整真人 UAT。
...
| V4~V6 集成进 smoke / preflight / reset | ⬜ 转 backlog | smoke 已为 `SMOKE_ADMIN_PASSWORD` 加"未设置即失败"快门（A 类安全债）；**是否需为师傅接口在总闸 / preflight 增闸门** —— 未在 P5-2 期间提出，转入 `docs/BACKLOG.md` 由后续阶段决定（**不阻塞阶段关闭**） |
| **P5-1 交付基线** | 🟢 **PASS** | 用户 2026-09-25 裁定；**正式交付基线 `297e728`**，此后不再改动以证 P5-1 |
| **真人走查（AT-16~19 / AT-22）** | 🟢 **P5-2 PASS** | 首次真人走查即通过入口 / 上传 / 收费 / 提交 / 一次性 Token 失效，**无操作阻断**；提出的规则反馈立为 DEV-82 并收口。见 `docs/PHASE-5-P5-2-UAT.md` |
| **DEV-82 说明条件必填**（真人 UAT 反馈的规则优化） | 🟢 **已收口**（`14c5b1a`） | `service_note`：`resolved` 可留空、其余四种必填、**未知新枚举默认必填**（fail-closed）；规则由服务端下发（`note_required`）。**不重开 P5-1** |
| 四组交付证据汇总 | 🟢 完成 | `docs/PHASE-5-P5-1-EVIDENCE.md`（Token 矩阵 / 上传矩阵 / Submit 前后快照 / 浏览器走查） |

> ✅ **Phase 5 已 PASS（用户 2026-09-25 裁定）—— 阶段关闭**：P5-1 的两项 HOLD 条件
> （① "1–6 张"口径定死 + 服务端权威校验；② `SMOKE_ADMIN_PASSWORD` 仓库级清理）均已关闭，
> P5-2 的唯一条件项 DEV-82 已收口。九支门禁全绿（submit **19** 为新基线）、secret audit 通过、
> 远端已发布（`297e728` 交付基线 + `14c5b1a` P5-2 收口修复）。
> **本文件的执行性状态已按 Phase 4 已验证的方法清扫完毕**（见 §14 关闭记录）——
> 历史 HOLD / 失败记录**保留原文并加最终结论批注**，未篡改历史。
>
> ⚠️ **阶段冻结**：Phase 5 关闭后**不再优化 Technician H5**。下一阶段从
> **`WAIT_STORE_CONFIRM`** 接棒（门店看到技师提交的结果 / 照片 / 报费信息，进行**确认或驳回**）——
> 即此前**刻意没有提前实现**的那一段闭环，见 §15。
...
> **状态**：🟢 **PASS**（用户 2026-09-25 裁定）—— 本阶段**唯一**的人类验收项，现已关闭。
> 工具包：`docs/PHASE-5-P5-2-UAT.md`（方法与判定 + §8.3 定向复验证据）+
> `docs/PHASE-5-P5-2-UAT-SHEET.md`（现场记录表）。
>
> **真人的真实轨迹（不美化，见 §14.1）**：首次走查即从短信形状 `/t/{token}` 用**真实手机**打开 →
> 上传照片 → 填回执 → 提交**成功** → 页面显示终态 → **再次打开同一链接失效**。
> ⇒ 两个关键结论成立：**① 核心链路可用、无操作卡点；② 一次性 Token 行为正确**。
> 同一次走查里，真人提出一条**规则反馈**：`resolved` 场景下"处理说明"不应强制填写
> —— 立为 **DEV-82**（口径见 §4.5），经**定向机器复测** + **最小真实浏览器复验**收口。
> ⚠️ **DEV-82 之后没有再跑第二轮完整真人 UAT**（用户明确不要求），
> **也不得把首次走查描述成"第一次就完美通过"** —— 它通过的是链路与 Token 行为，
> 留下了一条已如实登记的规则问题。
...
→ 本轮反馈的「说明字段应改为条件必填」属**业务规则优化**，单独立 **DEV-82** 定向收口（不重开 P5-1）。

> ⚠️ **本阶段（已按此执行）不做的事**：P5-2 **不新增功能**、**不改 `297e728`**。
> 阻断项 ⇒ 另立 DEV 编号（**不是**"重开 P5-1"）；非阻断项 ⇒ 进 backlog。
> 判定口径与 Phase 4-I 一脉相承（`docs/PHASE-4-I-UAT.md` §3）。

---

## 14. 关闭记录（用户 2026-09-25 裁定 · 🟢 PASS）

### 14.1 最终状态

```
P5-0  Routing & Environment Gate        PASS   (7b7e232)
P5-1  Technician API/H5/Security        PASS   (297e728)
P5-2  Mobile Human UAT                  PASS   (DEV-82: 14c5b1a)
Phase 5                                 PASS
```
...
|---|---|
| **裁定原文** | **「Phase 5：🟢 PASS / P5-2：🟢 PASS / Phase 5：🟢 PASS / 允许进入下一阶段。」** |
| **P5-1 正式交付基线** | **`297e728`** —— **不倒回去改写 P5-1 历史**；DEV-82 是 P5-2 的收口修复，独立成 commit |
| **P5-2 收口修复** | **`14c5b1a`**（25 文件）—— DEV-82 条件必填 + 文档口径同步 + 走查脚本留空模式 |
| **不再安排第二轮完整真人 UAT** | 理由：DEV-82 证据链已足够（服务端权威规则 + 前端消费服务端规则 + 19 项 submit 矩阵 + 反向/变异验证 + 真实 Chromium 空说明提交链路） |
| **阶段冻结** | 关闭后**不再优化 Technician H5** |

### 14.2 三个核心业务不变量（用户点名确认成立）

| # | 不变量 | 由什么保证 | 取证 |
|---|---|---|---|
| 1 | **照片上传成功 ≠ 服务提交成功** | 上传与提交是**两条独立接口**、各自的守卫与限额；上传只产照片行，**不推进任何状态**；提交才在单事务里推进 Visit/Ticket 并消费 Token | upload 矩阵（20 项）· submit 矩阵（19 项）· 真实浏览器 ③/⑤ 分步取证 |
| 2 | **技师提交成功 ≠ 工单完成** | 提交只到 `WAIT_STORE_CONFIRM`；`completed_at` / `reviewed_at` 必须仍为空；**不发**客户评价短信 | 证据 ③ 前后快照 · 真实浏览器库侧复核（`completed_at` / `reviewed_at` 均为空） |
| 3 | **技师提交后的唯一正确去向 = `WAIT_STORE_CONFIRM`** | M8 状态机迁移；**不得**直达 `CLOSED`；停止线核验：`actions/technician/` 与 `h5/src` 全量 grep `CLOSED` / 评价 Token / 评价短信，**命中只有注释** | 证据 ③ · `verify-technician-submit` A1/A3 · 停止线核验 |

### 14.3 DEV-82 作为**最终契约**（用户接受）

```
service_note：
  resolved                                      → 可留空（落库 NULL）
  need_followup / unresolved / customer_absent / other → 必填（缺 → 422 MISSING_SERVICE_NOTE）
  未知的新枚举                                   → 默认必填（fail-closed）
  上限 500 字
```

> 用户原话要点：**这个 fail-closed 设计比在前端复制一份枚举判断稳得多。**
> ⇒ 判定取「**可留空名单**」（`SERVICE_RESULT_NOTE_OPTIONAL = ['resolved']`）而非「必填名单」，
> 新增枚举若忘登记**默认按必填**处理，不会静默放开。规则**唯一事实来源在服务端**，
> 由 `GET /api/technician/visits/:token` 下发 `service_results[].note_required`。

### 14.4 本次关闭动作做了什么（文档清扫）

按 **Phase 4 已验证过的方法**做了一次 stage-closure doc sweep，重点检索
`Phase 5` / `P5-2` / `HOLD` / `PENDING` / `CONDITIONAL PASS` / `不得进入下一阶段` /
`不得进入 Phase 6` / `等待验收` / `待关闭`。处理原则：

- **当前执行性状态 → 改成 PASS**（头部状态块、阶段状态表、交付状态表、跨文档状态行）；
- **历史 HOLD / 失败 / 未关闭记录 → 保留原文，只加最终结论批注**（不篡改历史）；
- **真人 UAT 的真实轨迹完整保留**（见 §14.1 与 `docs/PHASE-5-P5-2-UAT.md` §8）：
  首次真的走查结果 + 真人提出的规则反馈 + DEV-82 + 最终 PASS —— 既不写成"第一次就完美通过"，
  也不写成"进行了第二轮完整真人 UAT"。

### 14.5 归属后续 backlog（**不重开 Phase 5**）

| # | 事项 | 归属 |
|---|---|---|
| 1 | 一次性凭证走查的**证据目录改用 `run_id` 归档、禁止覆盖已有 run**（第二次误执行应留下"Token 已消费"的独立证据，而不是覆盖第一次的成功证据） | `docs/BACKLOG.md` · 测试基础设施可靠性 |
| 2 | 是否为师傅接口在 smoke / preflight 增闸门（V4~V6 的遗留核项） | `docs/BACKLOG.md` · 运维/闸门覆盖 |
| 3 | 跨阶段历史文案残留：`scripts/verify-concurrency-phase2.mjs` 仍打印「Phase 2 仍为 HOLD」（Phase 2 早已补签 PASS）—— 本次**不动**，仅登记 | `docs/BACKLOG.md` · 文档/脚本口径漂移 |

---

## 15. 下一阶段（**从 `WAIT_STORE_CONFIRM` 接棒**）

> Phase 5 到此**冻结**，`Technician H5` 不再优化。下一阶段正是此前**刻意没有提前实现**的那一段闭环。

**Phase 6 — 门店确认 / 驳回 / 多次 Visit**（`docs/DEV-PLAN.md` §Phase 6；验收 AT-08 / AT-09 / AT-20 / AT-21 / AT-23）

起点就是本阶段留下的那个状态：**Ticket = `WAIT_STORE_CONFIRM`，Visit = `SUBMITTED` / `pending`**。
门店需要看到技师提交的**三样东西**并作出判断：

1. **处理结果**（`service_result` 五项枚举）与**处理说明**（DEV-82 条件必填，可能为空）；
2. **现场照片**（1~6 张，私有存储 + 受控读取 —— 门店侧要有合法的读取路径，**不得**把私有目录放开）；
3. **报费信息**（`is_charged` / `reported_charge_amount`）—— 确认时要能比对
   `confirmed_charge_amount`，不一致**必须填原因**（M9）。

**本阶段为下一阶段留下的接口/数据面**（已就位，无需迁移）：

- `service_visits` 的回执字段（`service_result` / `service_note` / `is_charged` /
  `reported_charge_amount` / `confirmed_charge_amount` / `submitted_at`）**全部已存在**；
- `store_confirm_status` 与 `store_confirm_note` / `store_confirmed_by` / `store_confirmed_at` 已存在
  （注意：`store_confirm_status` 自 Phase 4-A 起是 `visit_status` 的**派生字段**，
  **两套状态不得各自推进** —— 映射表 `VISIT_STATUS_TO_CONFIRM_STATUS`）；
- M9（`confirm` → `WAIT_FEEDBACK`，此时**才**发评价短信）/ M10（`reject` → 回 `PROCESSING`，
  **Visit 与照片全部保留**）的迁移已在 `docs/STATE-MACHINE.md` 定义。
- **照片读取的权限设计是下一阶段的第一个硬问题**：Phase 5 的读取路径是"**属主绑定 + 短时签名**"
  （`photoRefOf(id, tokenHash)`），门店侧是**登录态 + 门店数据隔离**的另一套口径 ——
  **不得**为了省事把 Phase 5 的 ref 机制复用到门店端，也不得把私有目录改成公开可直读。
