...
> | AT-01 / AT-02 / AT-03 等阶段内验收 | ✅ 通过（总闸 `scripts/smoke-test.mjs` 真机全绿；**项数随阶段演进，以脚本实际输出为准，本文不复写数字**） |
...
- ✅ **已达成的门槛**：`AT-03` 通过（门店 A 用户经**原生接口**与 `/api/svc` 两条路径都无法看到门店 B 工单，且 get 他店返回 404 而非 403）；事件必写通过；参数配置真机通过；三级权限与字段白名单通过（真机 `scripts/smoke-test.mjs` 冒烟全绿）。
...
node scripts/verify-config.mjs          # 期望 44/44

# 2) 插件生命周期离线校验（桩环境，不需要容器）
node scripts/verify-plugin-load.mjs     # 期望 57/57

# 3) 起容器
docker compose up -d

# 4) 真机端到端验收（--wait 会等应用就绪，首次启动约 1–3 分钟）
node scripts/smoke-test.mjs --wait 240  # 期望 76/76（Phase 3.1 五组重复单断言并入后）
