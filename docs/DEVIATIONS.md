...
| 根因②（**交付层，真正的元凶**） | nginx 对 `/static/plugins/` 发 `expires 7d;` + `Cache-Control: public, max-age=604800`，**且响应没有 ETag**；而插件产物 URL 形如 `/static/plugins/@local/service-ticket/dist/client/index.js?hash=b77ddccc` —— 那个 `?hash=` **不是内容哈希**：实测「改了源码 → 产物 md5 `edda70c1…` → `c42db789…` → `?hash=` 仍为 `b77ddccc`」⇒ **等价于 URL 里没有内容哈希**，缓存键在每次重建后仍然相同。⇒ **浏览器 7 天内不会回源**：插件重新构建后，真人浏览器**仍在跑旧产物**。<br>这解释了整轮的谜题——**为什么只有真人看得见 404**：<br>· 自动化探针每次 `--user-data-dir` 全新（空缓存）⇒ 永远拿最新产物 ⇒ **永远绿**；<br>· 真人用持久 profile ⇒ 被缓存粘住 ⇒ 一直跑那一版带 `/api/` 的旧产物。<br>⇒ **"自动化全绿"恰恰证明它测的不是真人的环境。** |
