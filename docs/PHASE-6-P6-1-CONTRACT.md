...
> **状态：🔒 FROZEN / 已履行（2026-09-25 定稿 → 2026-09-26 随 Phase 6 关闭）。**
> **§11 的 7 项开放项（O1~O7）已全部裁决**（见 §11.1）；**实现已交付并 PASS**
> （领域事务 + I12/I13 API，基线 **`c593bcd`**；审核 UI 见 P6-2，基线 `3ff8936`）。
> **Phase 6 整体 🟢 PASS（2026-09-26）**，契约进入**只读**状态 —— 语义不再重开讨论。
>
> ⚠️ **历史原貌（保留不改）**：本节原写「🟡 契约先行（2026-09-25 开工）· **待用户裁决 §11 的开放项后才进入实现**」，
> 以及「**本文定稿前，不写任何按钮、不写任何写路径**」—— 那是**开工时**的纪律（`docs/PHASE-6.md` §6.3：
> 按钮必须晚于事务契约与授权面）；该纪律**已履行完毕**，两项前提均已满足，故本节按**当前状态**改写。
>
...
> ⚠️ **`completed_at` 语义冻结**（用户 2026-09-25 拍板，无需另开 DEV）：它记录的是
> **「门店确认完成」时刻**（M9 的服务处理完成/门店确认完成时间），**不是**"整个 Ticket CLOSED 时间"，
> 也**不是**"客户评价完成时间"。Phase 7 做评价（`WAIT_FEEDBACK → CLOSED`，M12/M14）时，
> **不得**再次覆盖/误解释 `completed_at` —— 评价完成时刻应落在 `reviewed_at`
> （`service_tickets` 已有该列），两者语义分开：`completed_at` = 门店侧服务完成，
> `reviewed_at` = 客户侧评价完成。本阶段 confirm 写 `completed_at` 是正确的，保持不动。
...
| C16 | **reject 后 `reassign` 必须失败**（回 409，不是 500） | 单例（R1）。`NO_ACTIVE_VISIT` = 当前资源状态不允许 reassign，HTTP 409（状态冲突）比 422（参数问题）更合适，已按实现修正文档（2026-09-25，无需 DEV）。 |
...
| **C23b** | ⚠️ **注入点不得由生产请求参数开启** —— 只能由**测试环境显式触发**（进程级测试钩子 / 环境变量），请求体 / query / header **一律不认** | 反向用例：请求里带任何 `fault` / `inject` 参数 ⇒ **不得**触发回滚（该参数根本不被识别）；并断言默认路径上钩子恒为关闭 |
| **C26** | **真并发三组（§11.8）**：confirm×confirm / confirm×reject / reject×reject **同时**发出 ⇒ 每组**恰好一个 winner**，loser **409 + DB 最终真实状态**，且**只有一套** winner 的 Event/Token/幂等副作用 | 并发脚本（Promise.all 真并发，**禁止**串行模拟）+ **库内计数**取证。⚠️ **每组用各自独立的 fixture Ticket/Visit**（不复用同一 Visit 再"改状态复活"）；**winner 是谁不固定** —— 断言"**恰好一个成功 + 库终态与 winner 一致**"，**不写依赖调度顺序的脆弱断言** |
| **C24** | **幂等 `visitId` 参与冲突判定（L5）**：同 `actor+scene+request-id` 下，**同 visitId ⇒ replay**、**不同 visitId ⇒ conflict**（不得回放旧 Visit 结果） | 三条用例：同 Visit 重放 / 跨 Visit 同号 ⇒ **409 `IDEMPOTENT_VISIT_MISMATCH`** / confirm 与 reject 互换 scene（**不得互相命中**）。⚠️ **并发唯一索引兜底路径同样要遵守 visitId 维** |
| **C24b** | **正向断言（用户 2026-09-25 点名）**：**P6-1 新产生的 confirm/reject 幂等记录，`resource_id` 必须 = `visitId` 且非空** | 库内断言；**反向**（handler 忘传 `visitId` ⇒ 必须变红）—— 否则会**悄悄退化成兼容模式**（`resource_id=null` ⇒ 不校验 Visit 维，与"没加这一维"等价） |
...
| # | 锁点 | 冻结口径 |
|---|---|---|
| **L1** | **409 loser 是业务冲突，不是幂等 replay** | 两个员工对同一 `SUBMITTED`/待审 Visit 操作时：第一个成功；第二个**条件 UPDATE 影响行数 = 0** ⇒ **重读真实状态**并回 **409**。⚠️ **不得**因为第二个请求"恰好也是 confirm"就包装成"已经确认 ⇒ 算成功"。**只有相同幂等键的合法 replay** 才返回首次的原始成功结果。⚠️ 409 载荷**只回安全业务字段**（`visit_status` / `store_confirm_status` / `ticket_status` 等），**不得**把整个 Visit / Ticket ORM 对象序列化出去（会把内部列一次性暴露给客户端） |
